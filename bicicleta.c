...
int modificarBicicleta(eBicicleta listaDeBicicletas[], int cantidadDeBicicletas,eTipo listaDeTipos[],int cantidadDeTipos, eColor listaDeColores[],int cantidadDeColores)
{
    ...
    if(listaDeBicicletas != NULL && cantidadDeBicicletas>0 && listaDeTipos != NULL && cantidadDeTipos > 0 && listaDeColores != NULL && cantidadDeColores > 0)
    {
        ...
        idElegido = buscarBicicletaPorId(listaDeBicicletas,cantidadDeBicicletas,idElegido);
        while(idElegido == ERROR)
        {
        ...
        printf("\n");
        if(!mostrarBicicleta(listaDeBicicletas[idElegido],listaDeTipos,cantidadDeTipos,listaDeColores,cantidadDeColores))
        {
            printf("Error\n\n\n");
        }
        printf("\n");
        ...
        {
            bicicletaAuxiliar = listaDeBicicletas[idElegido];
            do
            {
                system("cls");
                printf("\n\n");
                ...
                case 'c':
                    ...
                    if(banderaSalirMenu == 's')
                    {
                        ...
                        {
                            listaDeBicicletas[idElegido] = bicicletaAuxiliar;
                        }
                    }
                    break;
                    system("pause");
...
int bajaDeBicicleta(eBicicleta listaDeBicicletas[], int cantidadDeBicicletas,eTipo listaDeTipos[],int cantidadDeTipos, eColor listaDeColores[],int cantidadDeColores)
{
    ...
    if(listaDeBicicletas != NULL && cantidadDeBicicletas>0 && listaDeTipos != NULL && cantidadDeTipos > 0 && listaDeColores != NULL && cantidadDeColores > 0)
    {
        ...
        getInt("Ingrese el id de la bicicleta que desea eliminar: ",&indice,"Error, ingrese un numero");

        indice = buscarBicicletaPorId(listaDeBicicletas,cantidadDeBicicletas,indice);
        while(indice == ERROR)
        {
            getInt("Error, ese id no existe en la lista.\nIngrese el id bicicleta que desea modificar: ",&indice,"Error, ingrese un numero");
        }
